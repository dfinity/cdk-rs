...
///
/// <div class="warning">
/// Previous CDK versions allow only one guard function to be specified with `guard` attribute.
/// A deprecation warning will be emitted if you use `guard` attribute.
/// In case both `guard` and `guards` are specified, the `guard` will be executed first.
/// </div>
///
