...
- Allow setting decoding quota for canister endpoints and inter-canister calls.
  * When defining canister endpoints, we add the following attributes: `#[update(decoding_quota = 10000, skipping_quota = 100, debug = true)]`
    ...
    - `debug = true` prints the instruction count and the decoding/skipping cost to the replica log, after a successful deserialization. `decoding_quota` and `skipping_quota` is logged only when you have already set a quota in the attributes. The debug mode is useful to determine the right quotas above. Developers can send a few large payload to the debugging endpoint and know the actual decoding cost.
