...
pub fn id() -> Principal {
    ...
    let len = unsafe { ic0::canister_self_size() as usize };
    let mut bytes = vec![0u8; len];
...
pub fn data_certificate() -> Option<Vec<u8>> {
    ...
    unsafe {
        ic0::data_certificate_copy(buf.as_mut_ptr() as isize, 0isize, n);
    }
    Some(buf)
}
...
pub fn is_controller(principal: &Principal) -> bool {
    ...
    unsafe { ic0::is_controller(slice.as_ptr() as isize, slice.len() as isize) != 0 }
}
