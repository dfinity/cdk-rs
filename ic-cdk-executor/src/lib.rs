...
thread_local! {
    static METHODS: RefCell<SlotMap<MethodId, MethodContext>> = RefCell::default();
    static TASKS: RefCell<HopSlotMap<TaskId, Task>> = RefCell::default();
    static PROTECTED_WAKEUPS: RefCell<SecondaryMap<MethodId, VecDeque<TaskId>>> = RefCell::default();
    static MIGRATORY_WAKEUPS: RefCell<VecDeque<TaskId>> = const { RefCell::new(VecDeque::new()) };
    static CURRENT_METHOD: Cell<Option<MethodId>> = const { Cell::new(None) };
...
fn enter_current_method<R>(method_guard: MethodHandle, f: impl FnOnce(MethodId) -> R) -> R {
...
impl MethodHandle {
    fn for_method(method_id: MethodId) -> Self {
