...
This tutorial provides a simple example of how you can increment a counter by calling functions on a deployed smart contract canister.
...
* Deploy the smart contract canister on the local {IC}.
* Invoke the smart contract methods to increment then read the value of a counter.
...
You need to modify the default settings in the `+dfx.json+` configuration file to build Rust dapps that run on the {IC} as smart contract canisters.

To modify the `+dfx.json+` configuration file:

. Check that you are still in the root directory for your project, if needed.
. Open the `+dfx.json+` configuration file in a text editor.
. Replace the `+canisters.rust_counter+` settings with settings for building a smart contract canister using the `+cargo build+` command.
+
...
Candid is an interface description language (IDL) for interacting with smart contract canisters running on the Internet Computer.
...
Before you can build the `+rust_counter+` project, you need to connect to the {IC} either running locally in your development environment or the decentralized {IC} mainnet.
...
After successfully deploying the smart contract canister, you can test the canister by invoking the functions it provides.
For this tutorial:
