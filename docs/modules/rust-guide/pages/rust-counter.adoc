...
In the link:rust-quickstart{outfilesuffix}[Rust Canister Developer Quick Start], you saw that creating a new project adds several template files to your project directory much like when you create a new Rust package using the `+cargo new+` command.
...
Creating a new project creates a default `+src+` directory with a template `+main.mo+`. We'll replace this default program with a Rust library `+lib.rs+` package and `+Cargo.toml+` files. 
...
=== Add Cargo settings for the main program
...
----
+
This command creates a `+src+` directory with a library (`+lib.rs+`) package and a `+Cargo.toml+` file in the `+src/rust_counter+` directory.
...
The next step is to replace the default source code in the `+src/rust_counter/src/lib.rs+` file with a Rust program that declares the `+COUNTER+` variable and implements the `+increment+`, `+get+`, and `+set+` functions.

To modify the default template source code:

. Check that you are still in the root directory for your project, if needed.
. Open the `+src/rust_counter/src/lib.rs+` file in a text editor and delete the existing content.
...
. Check that you are still in the root directory for your project, if needed.
. Create a new file named `+counter.did+` in the `+src/rust_counter+` directory.
. Open the `+src/rust_counter/counter.did+` file in a text editor, then copy and paste the following `+service+` definition for the `+increment+`, `+get+`, and `+set+` functions:
+
[source,did]
----
service : {
...
Executing 'cargo build --target wasm32-unknown-unknown --package  rust_counter --release'
  ...
  ...
     Compiling ic-cdk v0.2.4
   Compiling ic-cdk-macros v0.2.4
...
To optimize the resulting canister:
...
. Create a release directory within the `+src/rust_counter+` directory by running the following command:
+
[source,bash]
----
mkdir -p src/rust_counter/target/wasm32-unknown-unknown/release/
----
. Optimize the code within the `+target/rust_counter+` directory by running the following command:
+
[source,bash]
----
ic-cdk-optimizer target/wasm32-unknown-unknown/release/rust_counter.wasm -o target/wasm32-unknown-unknown/release/rust_counter-opt.wasm
----
