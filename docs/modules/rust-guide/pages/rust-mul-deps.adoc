...
. Create a new project named `+multiply_deps+` by running the following command:
+
[source,bash]
----
...
Because this sample project is going to consist of two programs-the {native} program and the {lang} program—you need to modify the default `+dfx.json+` configuration file to include information for a {lang} program.
...
+
The `+dependencies+` setting enables you to import function for the specifies program, in this case the `+multiply_deps+` program written in {native}.
