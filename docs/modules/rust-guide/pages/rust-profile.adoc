...
* The `+getSelf+` function returns the profile associated with the function caller.
...
By adding a second program, we can demonstrate service interoperability and composability.
...
----
. Save your change and close the `+dfx.json+` file to continue.
. Open the `+Cargo.toml+` file if you created for the project and add the `+profile_interoperation+` package as a member.
+
For example:
+
[source,toml]
