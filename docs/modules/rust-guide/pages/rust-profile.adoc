...
You need to modify the default settings in the `+dfx.json+` configuration file to build Rust programs that run on the {IC} as smart contract canisters.
...
Before you can build the `+rust_profile+` project, you need to connect to the {IC} either running locally in your development environment or the decentralized {IC} mainnet.
...
. In the terminal that displays network operations, press Control-C to interrupt the local {IC} process.
