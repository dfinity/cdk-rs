...
This tutorial provides a simple example of how you can use the Rust CDK interfaces and macros to simplify writing Dapps in Rust for the {IC} blockchain network.
...
You need to modify the default settings in the `+dfx.json+` configuration file to build Rust programs that run on the {IC} blockchain network as smart contract canisters.
...
Now that you have the files in place for your Rust Dapp, we can replace the template `+main.mo+` program with the Rust Dapp we want to deploy on the {IC} blockchain network.
...
Before you can build the `+rust_profile+` project, you need to connect to the {IC} network either running locally in your development environment or the decentralized {IC} mainnet.
