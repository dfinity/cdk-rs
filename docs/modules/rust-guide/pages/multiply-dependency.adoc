...
One common approach to Dapp design is to calculate or store data in one smart contract that you can then use in another smart contract. 
This ability to share and use functions defined in different smart contract canisters, even if the underlying smart contracts are written in different languages, is an important strategy for building Dapps to run the {IC} blockchain network.
...
For this tutorial, both smart contract canisters are in the same project. 

* The {native} smart contract canister creates an actor with a `+cell+` variable to contain the current value that results from an operation.
...
* The {lang} smart contract canister provides a `+read+` function that is a simple query that returns the current value of the `+cell+` variable.
...
You need to modify these default files and add some Rust-specific files to your project before you can build smart contract canisters that run on the {IC} blockchain network.
...
* <<Replace the default {native} smart contract canister>>
* <<Replace the default {lang} smart contract canister>>
...
+
The `+dependencies+` setting enables you to import functions from one smart contract canisters for use in another smart contract canister. For this tutorial, we want to import a function from the `+multiply_deps+` canister—written in {native}—and use it from the `+rust_deps+` canister written in Rust.
...
Creating a new project creates a default `+src/multiply_deps+` directory with a template `+main.mo+` for our {native} smart contract canister. 
For this tutorial, we want to add the files for building a {lang} smart contract canister.
...
Now that we have the {native} smart contract canister that the {lang} smart contract canister depends upon, let's add the {lang} canister to the project.

To replace the default {lang} smart contract canister:
...
Candid is an interface description language (IDL) for interacting with smart contract canisters running on the Internet Computer blockchain network.
Candid files provide a language-independent description of a smart contract canister's interfaces including the names, parameters, and result formats and data types for each function a canister defines.

By adding Candid files to your project, you can ensure that data is properly converted from its definition in Rust to run safely on the {IC} blockchain netowrk.
...
Before you can build the project, you need to connect to the {IC} network either running locally in your development environment or the decentralized {IC} mainnet.
...
. Call the `+read+` function from the {native} caniser, which reads the current value of the `+cell+` variable on the deployed canister:
+
[source,bash]
----
